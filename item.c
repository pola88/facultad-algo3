...
/**@brief Funcion que cambia la clave de un item o la inicia.
   @param item: Puntero al item que queiro cambiar o iniciar la clave.
   @param key: puntero a la calve por la que quiero iniciar o cambiar el item.
*/
...
void item_init(Item* item, char* key, char* value, const size_t size) {
  ...
  item->value = (Data*) malloc(sizeof(Data));
